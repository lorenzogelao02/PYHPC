...
if __name__ == '__main__' : 
    ...
    master_file_path = "output/task5/all_workers_results.txt"
    with open(master_file_path, "w") as out_file:
        out_file.write("--- Scaling Benchmark Results ---\n")
    ...
    # --- 3. PRINT RESULTS ---
        ...
        with open(master_file_path, "a") as out_file:
            out_file.write(f"Time taken with {num_workers:02d} workers: {time_taken:.2f} seconds\n")
            
        print(f"Finished evaluating {num_workers} workers!")        
    ...
    plt.plot(worker_counts, speedups, marker='o', color='crimson', linewidth=3, markersize=8)

    # Annotate each point
    for i, txt in enumerate(speedups):
        plt.annotate(f"{txt:.2f}x", 
                     (worker_counts[i], speedups[i]), 
    ...
    plt.title('Speedup Growth as Workers Increase', fontsize=16, fontweight='bold')
    ...
    plt.savefig('output/task5/speedup_plot.png', dpi=300)
    print("Plot successfully saved to output/task5/speedup_plot.png!")