...
The CuPy vectorized solution processed 2 floorplans in **4.56 seconds**, resulting in an average of **2.28 seconds per floorplan**. This is a monumental speedup compared to the original pure Python/NumPy CPU reference implementation, but notably, it is about **2x slower** than our hand-written Numba CUDA implementation (which averaged 1.03s per floorplan).
...
In contrast, our Numba implementation compiled the entire arithmetic calculation and the mask condition into a **single fused kernel**, entirely eliminating the launch overhead and temporary array allocations. This brilliantly highlights the tradeoff between the simplicity of writing vectorized NumPy-like code and the raw performance of writing explicit low-level GPU kernels!
